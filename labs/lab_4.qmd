...
The output will produce the usual SPSS output window with the descriptive statistics for each assessment, as well as a new column (variable) in the main data set called `ZExtroversion_Score`. This new variable contains the $z-scores$ for each applicant's extroversion score.
...
Who are the top ten most extroverted individuals. Record their `Applicant_ID`'s and `ZExtroversion_Scores`.
:::
